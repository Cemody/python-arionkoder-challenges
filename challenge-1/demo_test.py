...
"""Helper to measure memory and execution time"""
    print(f"\n=== {test_name} ===")
...
"""Test core streaming data processing functionality"""
...
"""Test handling of variable-rate data streams"""
    ...
    def run_test():
        ...
        for i in range(burst_count):
            payload = {
                'events': [
                    {'batch_id': f'burst_{i}', 'request_num': i, 'type': 'burst_test'}
                ]
            }
        ...
        test_sizes = [10, 100, 500, 10]                                     
        for i, size in enumerate(test_sizes):
            payload = {
                'data': {
                    'records': [
                        {'payload_size': f'size_{size}', 'record_id': j, 'value': j}
                        for j in range(size)
                    ]
                }
            }
        ...
        return successful_requests == burst_count
    
    return run_test()


def test_constant_memory_usage():
"""Test constant memory usage with increasing data volume"""
    client = TestClient(app)
    ...
    def run_test():
        ...
        for size in data_sizes:
            ...
            if response.status_code == 200:
                ...
                print(f"📊 Size {size}: {processing_time:.2f}ms, "
                      f"{result['processed_records']} groups, "
                      f"aggregation keys: {len(result['aggregation']) if result['aggregation'] else 0}")
        ...
        return len(processing_times) == len(data_sizes)
...
"""Test data transformation capabilities"""
    ...
    def run_test():
        ...
        if response.status_code == 200:
            ...
            print(f"🔒 Sensitive fields filtered during projection")
            return True
...
"""Test system status and health endpoints"""
    ...
    def run_test():
        ...
        return health_response.status_code == 200 and status_response.status_code == 200
